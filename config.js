...
const BANK_CONFIG = {
  ...
  // Checks environment / local server ports (Django 8000 / Node 5000 / production)
  API_BASE_URL: (() => {
    if (typeof window !== 'undefined') {
      const loc = window.location;
      if (loc.hostname === 'localhost' || loc.hostname === '127.0.0.1') {
        if (loc.port === '8000') return `${loc.origin}/api/v1`;
        return 'http://127.0.0.1:8000/api/v1'; // Default Django backend
      }
      if (loc.origin.includes('http') && !loc.origin.includes('github.io')) {
